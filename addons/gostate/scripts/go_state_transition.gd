...
@export_tool_button("Print Condition Result") var test_condition_tool_button = Callable(
	func():
...
func _get_configuration_warnings() -> PackedStringArray:
	...
	if not condition.is_empty():
		if condition_input_node != null:
			_set_condtion_node_input_properties()
			var condition_expression_error = _condition_expression.parse(
				condition,
				 _condition_inputs.keys()
			)
